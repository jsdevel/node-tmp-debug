...
function coerceConfig(config) {
  ...
  config.ignore.files = config.ignore.files || [];
  config.ignore.functions = config.ignore.functions || [];
}
...
function stringifyArg(arg) {
  switch (typeof arg) {
    ...
    default:
      ...
      return util.inspect(arg, {colors: true, depth: 15});
  }
}

function canLog(config, stack) {
  var loggable = true;

  if (config.ignore.files.length) {
    var fileName = stack[1].getFileName();
    loggable = !config.ignore.files.some(function(file) {
      return endsWith(fileName, file);
    });
  }

  if (loggable && config.ignore.functions.length) {
    var fnName = stack[1].getFunctionName();
    loggable = !config.ignore.functions.some(function(fn) {
      return endsWith(fnName, fn);
    });
  }
